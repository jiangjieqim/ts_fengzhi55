...
void main()
{
    ...
    //主要效果算法
	vec3 worldRefl = reflect(normal, viewDir);
	
    vec4 reflcol = texture(u_TextureCube, worldRefl);
    reflcol *= difTexColor.a;     

	vec3 emission = reflcol.rgb * u_ReflectColor.rgb;
	float alphas = reflcol.a * u_ReflectColor.a;
    ...
    //最终效果
	gl_FragColor.rgba += vec4(emission, alphas);
