...
import { EventGamePassMonthProxy, EventGamePassPackProxy } from "../proxy/ZhanLingProxy";
import { ZhanLingCtl } from "./ctl/ZhanLingCtl";

export class ZhanLingItem extends ui.views.zhanling.ui_zhanLingItemUI{
    ...
    private _cfg:Configs.t_Event_GamePass_Month_dat;
    public setData(value:stBattlePassCard,index:number){
        ...
        if(index >= ZhanLingModel.Ins.monthList.length - 1){
            ...
            let nCfg:Configs.t_Event_GamePass_Month_dat = EventGamePassMonthProxy.Ins.GetDataById(va.fid);
