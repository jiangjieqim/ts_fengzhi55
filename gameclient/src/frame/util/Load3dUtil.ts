...
export class Load3dUtil {
     ...
     */
    public static getScene3DChild<T>(scene3d, childName: string): T {
        let ms = scene3d.getChildByName(childName) as T;
        if (ms) {
            return ms;
        }
        LogSys.Error("Error:获取场景物体失败");
        return null;
    }
    ...
    public static getModelChildByName<T>(fatSP, childName): T {
        ...
        LogSys.Error("Error:获取模型子物体信息错误");
        return null;
    }
    ...
    public static replaceModel(targetSP, mainSP) {
        if (!targetSP || !mainSP) {
            LogSys.Error("Error:替换或被替换模型信息错误");
            return null;
        }
        if (targetSP.parent) {
    ...
    public static getMaterials(scene3d): any {
        ...
        var container = Load3dUtil.getScene3DChild<Laya.MeshSprite3D>(scene3d, "MatContainer");
        if (!container) {
            LogSys.Error("Error:材质容器错误或不存在");
            return null;
        }
    ...
    public static getMaterialByName(matArrary, matName): Laya.BlinnPhongMaterial {
        if (!matArrary) {
            LogSys.Error("Error:材质哈希表信息错误");
            return null;
        }
        if (!matArrary[matName]) {
            LogSys.Error("Error:指定哈希表内不存在[" + matName + "]材质");
            return null;
        }
